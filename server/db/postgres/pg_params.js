...
PgParams.prototype.insert = function (condition_id, html_id, param, callback, errback) {
    ...
    var db = new PG(
        function(){
            db.transact(
                ...
                function (res) {
                    db.transact(
                        "SELECT currval(pg_get_serial_sequence('params','params_id'))",
                        [],
                        function(res){
                        ...
                        function(err){
                            console.log('PgParams.prototype.insert 1');
                            console.log(err);
                        },
                        true)
                },
                function (err) {
                    console.log('PgParams.prototype.insert 2');
                    console.log(err); 
                }
            );
        },
        function(err){
            console.log('PgParams.prototype.insert 3');
            console.log(err);
        }
    );
}
